...
export const About = () => {
  ...
  return (
    <div className="pt-36 pb-20 px-6 max-w-7xl mx-auto min-h-screen space-y-14 md:space-y-24">
      {/* Title Header */}
      <section className="text-center">
        <span className="text-brand-gold uppercase tracking-hero text-[10px] font-black block">{t('about.heritage.teaser')}</span>
        <h1 className="text-4xl max-[389px]:text-3xl md:text-6xl font-editorial-title mt-2 mb-4">
          {t('about.heritage.title')} <span className="font-bold italic gold-foil-text">{t('about.heritage.titleGold')}</span>
        </h1>
        <p className="text-sm md:text-xs text-foreground/50 max-w-xl mx-auto leading-relaxed font-light italic">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
        <div className="space-y-6">
          ...
          <p className="text-sm md:text-xs text-foreground/60 leading-relaxed font-light">
            {t('about.story.desc')}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 pt-4">
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs md:text-[10px] text-foreground/45 mt-1">{t('about.story.stat1.desc')}</p>
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs md:text-[10px] text-foreground/45 mt-1">{t('about.story.stat2.desc')}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* History Timeline */}
      <section className="py-8 md:py-12 border-t border-b border-foreground/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {historyTimeline.map((item, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bento-card glow-tracer p-8 flex flex-col justify-between min-h-[250px] relative border border-foreground/5"
            >
              <div>
                ...
                <p className="text-xs md:text-[10px] text-foreground/50 leading-relaxed italic">{t(`about.timeline.${item.id}.desc`)}</p>
              </div>
      ...
      <section className="py-8 md:py-12">
        <div className="text-center mb-10 md:mb-16">
          ...
          <p className="text-sm md:text-xs text-foreground/50 mt-3 max-w-sm mx-auto leading-relaxed italic">
            {t('about.team.desc')}
          </p>
        </div>
          ...
          {artisans.map((artisan, idx) => (
            ...
            >
              <div className="w-24 h-24 rounded-full overflow-hidden mx-auto mb-6 border border-brand-gold/20 bg-foreground/5">
                <img src={artisan.avatar} alt={artisan.name} width={300} height={300} loading="lazy" decoding="async" className="w-full h-full object-cover" />
              </div>
      ...
      {/* Brand values card */}
      <section className="bento-card p-8 sm:p-10 md:p-16 bg-brand-gold text-black text-center relative overflow-hidden group shadow-xl shadow-brand-gold/10">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1540555700478-4be289fbecef?q=60&w=600')] md:bg-[url('https://images.unsplash.com/photo-1540555700478-4be289fbecef?q=80&w=1200')] bg-cover bg-center opacity-10 group-hover:scale-102 transition-all duration-1000 ease-out" />
        <div className="relative z-10 max-w-2xl mx-auto space-y-6">
          <Sparkles className="w-8 h-8 mx-auto text-black motion-safe:animate-pulse-slow" />
          ...
          <p className="text-xs md:text-[11px] leading-relaxed max-w-md mx-auto font-semibold opacity-75">
            {t('about.footer.desc')}
          </p>
        </div>
      </section>
